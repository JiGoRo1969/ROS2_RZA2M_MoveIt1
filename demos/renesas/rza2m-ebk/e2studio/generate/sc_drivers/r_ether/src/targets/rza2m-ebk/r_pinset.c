...
{
    if ((ETHER_SC_TABLE[0].pin.p_config_table != NULL) && (ETHER_SC_TABLE[0].pin.count > 0))
    {
        int_t gpio_handle = (-1);
        ...
        if ((-1) != gpio_handle)
        {
            ...
            direct_control(gpio_handle, CTL_GPIO_INIT_BY_TABLE, (st_r_drv_gpio_pin_init_table_t *) &ETHER_SC_TABLE[0].pin);
