...
{
    if (ETHER_SC_TABLE[0].config.ether_ch0_en == 1)
    {
        if (PORT_CONNECT_ET0 == (connect & PORT_CONNECT_ET0))
        {
            if (ETHER_SC_TABLE[0].config.ether_mode == 0)
            {
            	/* CH0 MII */
            	GPIO.PFENET.BIT.PHYMODE0 = 1;
            }
			else if (ETHER_SC_TABLE[0].config.ether_mode == 1)
			{
				/* CH0 RMII */
    ...
    if (ETHER_SC_TABLE[0].config.ether_ch1_en == 1)
    {
