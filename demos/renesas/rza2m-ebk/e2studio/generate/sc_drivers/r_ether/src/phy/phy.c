...
int16_t phy_init (uint32_t ether_channel)
{
    ...
    if (phy_opened == 1)
    {
        ...
        do
        {
            ...
            count++;
        } while ((reg & PHY_CONTROL_RESET) && (count < ETHER_SC_TABLE[0].config.ether_phy_delay_reset));
    }
    else
    {
        phy_opened = 1;
    ...
    if (count < ETHER_SC_TABLE[0].config.ether_phy_delay_reset) {
...
void phy_start_autonegotiate (uint32_t ether_channel, uint8_t pause)
{
    ...
    phy_write(ether_channel, PHY_REG_CONTROL, (PHY_CONTROL_AN_ENABLE | PHY_CONTROL_FULL_DUPLEX |
...
static void phy_reg_read (uint32_t ether_channel, uint16_t *pdata)
{
    ...
    i = 16;
    while (i > 0) {
        for (j = ETHER_SC_TABLE[0].config.ether_phy_mii_wait; j > 0; j--) {
