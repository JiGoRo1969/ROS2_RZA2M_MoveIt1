...
#else
static descriptor_t rx_descriptors[ETHER_CHANNEL_MAX][ETHER_CFG_EMAC_RX_DESCRIPTORS] __attribute((section("UNCACHED_BSS"),aligned(32)));
static descriptor_t tx_descriptors[ETHER_CHANNEL_MAX][ETHER_CFG_EMAC_TX_DESCRIPTORS] __attribute((section("UNCACHED_BSS"),aligned(32)));
#endif
 ...
 * ETHER_SC_TABLE[0].config.ether_bufsize (the size of one buffer).
 * The ETHER_SC_TABLE[0].config.ether_bufsize and EMAC_NUM_BUFFERS are defined by macro in the file "r_ether_private.h".
...
void R_ETHER_Initial (void)
{
    ...
    for (i = 0; i < ETHER_CHANNEL_MAX; i++) {
        ...
        if (ETHER_SC_TABLE[0].config.ether_use_linksta == 0)
        {
    ...
    g_eth_control_ch[0].phy_address = ETHER_SC_TABLE[0].config.ether_ch0_phy_address;
    g_eth_control_ch[1].phy_address = ETHER_SC_TABLE[0].config.ether_ch1_phy_address;
...
ether_return_t R_ETHER_Open_ZC2 (uint32_t channel, const uint8_t mac_addr[], uint8_t pause)
{
    ...
    if ((ETHER_SC_TABLE[0].config.ether_ch0_en == 1) && (ETHER_SC_TABLE[0].config.ether_ch1_en == 0))
    {
        R_INTC_RegistIntFunc(INTC_ID_ETHER_EINT0, ether_eint0);
    }
    else if ((ETHER_SC_TABLE[0].config.ether_ch0_en == 0) && (ETHER_SC_TABLE[0].config.ether_ch1_en == 1))
    {
    ...
    if (R_PHY_OK == phy_ret) {
        ...
        if (ETHER_SC_TABLE[0].config.ether_use_linksta == 1)
        {
...
#if (ETHER_USE_MMU)
    p_rx_desc = (descriptor_t *)(((uint32_t)p_rx_desc) + ETHER_SC_TABLE[0].config.ether_mmu_uncached_diff);
#endif
    if (NULL == pbuf) {
        return ETHER_ERR_INVALID_PTR;
    }
        ...
        if (RACT != (p_rx_desc->status & RACT)) {
            ...
            if (ETHER_ERR_MC_FRAME != ret) {
                if (RFE == (p_rx_desc->status & RFE)) {
                     ...
                     */
#if (ETHER_USE_MMU)
                    /* R_MMU_PAtoVA(p_rx_desc->buf_p, pbuf); */
                    (*pbuf) = (void*)(((uint32_t) p_rx_desc->buf_p) + ETHER_SC_TABLE[0].config.ether_mmu_uncached_diff);
#else
...
#if (ETHER_USE_MMU)
        R_MMU_VAtoPA((uint32_t)p_rx_desc->next, (uint32_t*)&papp_rx_desc[channel]);
#else
        papp_rx_desc[channel] = p_rx_desc->next;
#endif
    }
    pedmac_adr = g_eth_control_ch[channel].pedmac;
...
#if (ETHER_USE_MMU)
    p_tx_desc = (descriptor_t*)(((uint32_t)p_tx_desc) + ETHER_SC_TABLE[0].config.ether_mmu_uncached_diff);
#endif
    if (NULL == pbuf) {
        return ETHER_ERR_INVALID_PTR;
    }
...
#if (ETHER_USE_MMU)
    /*R_MMU_PAtoVA( papp_tx_desc[channel]->buf_p, pbuf);*/
    (*pbuf) = (void*)((p_tx_desc->buf_p) + ETHER_SC_TABLE[0].config.ether_mmu_uncached_diff);
#else
...
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t)p_tx_desc->next, (uint32_t*)&papp_tx_desc[channel]);
#else
...
void R_ETHER_LinkProcess (uint32_t channel)
{
    ...
    if (ETHER_FLAG_ON_LINK_ON == lchng_flag[channel]) {
        ...
        else if (ETHER_SC_TABLE[0].config.ether_use_linksta == 0)
        {
           /*
            * The status of the LINK signal became "link-up" even if PHY-LSI did not detect "link-up"
            * after a reset. To avoid this wrong detection, processing in R_ETHER_LinkProcess has been modified to
...
ether_return_t R_ETHER_WakeOnLAN (uint32_t channel)
{
    ...
    if (ETHER_SUCCESS == ret) {
        if (ETHER_SC_TABLE[0].config.ether_use_linksta == 1)
        {
            ...
            if (ETHER_SC_TABLE[0].config.ether_link_preset == petherc_adr->PSR.BIT.LMON) {
                ret = ETHER_SUCCESS;
            } else {
                ret = ETHER_ERR_OTHER;
            }
...
#if (ETHER_USE_MMU)
        R_MMU_VAtoPA((uint32_t)(&(ether_buffers[channel].buffer[i][0])),(uint32_t*)&address);
        pdescriptor->buf_p = (uint8_t *)address;
#else
...
#if (ETHER_USE_MMU)
        R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel][(i + 1)], (uint32_t*)&(pdescriptor->next));
#else
...
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel][0], (uint32_t*)&(pdescriptor->next) );
#else
    pdescriptor->next = (descriptor_t *) &rx_descriptors[channel][0];
#endif
    /* Initialize application receive descriptor pointer */
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t)&rx_descriptors[channel][0], (uint32_t*)&papp_rx_desc[channel] );
#else
...
#if (ETHER_USE_MMU)
        R_MMU_VAtoPA((uint32_t)&(ether_buffers[channel].buffer[(ETHER_CFG_EMAC_RX_DESCRIPTORS + i)][0]), (uint32_t*)&address);
        pdescriptor->buf_p = (uint8_t *) address;
#else
...
#if (ETHER_USE_MMU)
        R_MMU_VAtoPA((uint32_t) &(tx_descriptors[channel][(i + 1)]), (uint32_t*)&pdescriptor->next);
#else
...
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t) &tx_descriptors[channel][0], (uint32_t*)&pdescriptor->next);
#else
...
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t) &tx_descriptors[channel][0], (uint32_t*)&papp_tx_desc[channel]);
#else
...
static void ether_config_ethernet (uint32_t channel, const uint8_t mode)
{
...
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t)papp_rx_desc[channel],(uint32_t*)&address);
    ...
    /* Casting the pointer to a uint32_t type is valid because the Renesas Compiler uses 4 bytes per pointer. */
#if (ETHER_USE_MMU)
    R_MMU_VAtoPA((uint32_t)papp_tx_desc[channel],(uint32_t*)&address);
...
static uint8_t check_mpde_bit (void)
{
    if ( (ETHER_SC_TABLE[0].config.ether_ch0_en + ETHER_SC_TABLE[0].config.ether_ch1_en) == 1)
    {
        /* The MPDE bit can be referred to only when ETHERC operates. */
        if ( (ETHER_MODULE_USE == etherc_edmac_power_cont[0])
                && (((ETHER_SC_TABLE[0].config.ether_ch0_en == 1)
