...
#define ACTIVE_ETHER_CHANNEL ETHER_CHANNEL_0
...
uint8_t *buffer_pointer;
    ...
    for( ;; )
    {
        ...
        xBytesReceived = R_ETHER_Read_ZC2(ACTIVE_ETHER_CHANNEL, (void **)&buffer_pointer);
...
static int16_t SendData( uint8_t *pucBuffer, size_t length )//TODO complete stub function
{
    ...
    ret = R_ETHER_Write_ZC2_GetBuf(ACTIVE_ETHER_CHANNEL, (void **) &pwrite_buffer, &write_buf_size);

	if (ETHER_SUCCESS == ret)
	{
		if (write_buf_size >= length)
		{
			memcpy((void *)pwrite_buffer, pucBuffer, length);
		}
