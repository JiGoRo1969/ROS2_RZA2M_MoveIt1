        ...
        size_t agent_list_size)
{
    ...
    {
        for(uint32_t a = 0; a < attempts && !found; ++a)
        {
            for(size_t i = 0; i < agent_list_size && !found; ++i)
            {
            ...
            while(0 < poll && !found)
            {
                found = listen_info_message(&transport, poll, &callback);
        ...
        CallbackData* callback)
{
    ...
    if(received)
    {
        ...
        {
            bResult = read_info_message(&ub, callback);
        }
    }
        ...
        CallbackData* callback)
{
    ...
    if(uxr_deserialize_INFO_Payload(ub, &payload))
    {
        ...
        {
            well_read = process_info(callback, transport);
        }
    }
        ...
        TransportLocator* locator)
{
    ...
    if(locator->format == ADDRESS_FORMAT_MEDIUM)
    {
        ...
        uint8_t* pByteAddr = &locator->_.medium_locator.address[0];
        sprintf( agent_ip, "%d.%d.%d.%d", *(pByteAddr), *(pByteAddr+1), *(pByteAddr+2), *(pByteAddr+3) );

        uxrAgentAddress address = {agent_ip, locator->_.medium_locator.locator_port};
